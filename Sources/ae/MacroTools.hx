...
class MacroTools
{
	 ...
	 */
	public static macro function buildMap(typePath:String, reverse:Bool = false, ?toInt:Bool = false, ?exclude:Array<String>):Expr
	{
		...
		if (!reverse && !toInt)
			...
			finalExpr = values.map(function(v) return macro $v{v.value} => $v{v.value});
		else
